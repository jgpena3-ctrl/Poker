"""
Quick test: reproduce mano2 frames through LiveRecorder and show all state.
"""
import json, os, sys
import numpy as np
...
def load_frame(num):
    path = os.path.join(JUEGO_DIR, f'mano2 ({num}).png')
...
for frame_num in range(11):
    ...
    # Show assigned positions before processing
    pos_map = recorder._get_pid_map(state)
    print(f'  Positions (pid->pos): {pos_map}')
    inv_map = recorder._get_pos_map(state)
    print(f'  Inverse (pos->pid): {inv_map}')

    rec_log = []
    recorder.process_frame(state, img_arr, log=lambda msg: rec_log.append(msg) or print(msg))

    if recorder.hand:
        cur = recorder.hand['current_street']
        print(f'  current_street={cur}')
        print(f'  inactive={recorder._inactive_players}')
        print(f'  actions so far: {json.dumps(recorder.hand["streets"][cur]["actions"], indent=2)}')
        # Show all streets' actions
        for sn in ['preflop', 'flop', 'turn', 'river']:
            acts = recorder.hand['streets'][sn]['actions']
            if acts:
                print(f'    {sn}: {acts}')

# If hand not closed, force it
if recorder.hand:
    print('\n>> Forzando cierre')
